...
def main():
    ...
    # 原稿133のように、話者ラベル（`名前「台詞」`）を使わず、冒頭の人物確認メモと
    # AA注記だけで話者を示す原稿がある。そういう原稿では①の判定が効かないので警告する。
    ラベル数 = {}
    for 名 in sorted(os.listdir(原稿ディレクトリ)):
        m = re.match(r'(\d{3})_.*\.txt$', 名)
        if not m:
            continue
        t2 = 読む(os.path.join(原稿ディレクトリ, 名))
        ラベル数[int(m.group(1))] = len(re.findall(r'^[^\s「（(【\[［#＃]{2,16}[「（]', t2, re.M))
    薄い = [n for n, c in sorted(ラベル数.items()) if c < 20]
    ...
    if 薄い:
        print()
        print('※ 話者ラベルがほとんど無い原稿: %s' % ' '.join('%03d(%d本)' % (n, ラベル数[n]) for n in 薄い))
        print('   この原稿では①の判定が効かない。話者は冒頭の人物確認メモとAA注記で示されるため、')
        print('   ②の言及走査と、原稿を実際に読むことで補う必要がある。')
